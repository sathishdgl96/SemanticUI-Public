...
def test_a_document_saved_before_announcements_moved_still_opens(db):
    """The key lived on the dashboard once. Refusing it now would make a
    saved dashboard unopenable because a field moved somewhere better."""
    sess = create_session(db, account="ACME", user="ALICE", mode="dev")
    ws = workspace(db, sess.user_id)
    made = dashboard(db, sess.user_id, ws)
    ...
    service.update_dashboard(
        ...
        str(made.id),
        {
            "schemaVersion": 1,
            "name": "Ops",
            "announcement": "Written before the move.",
            "tiles": [],
        },
    )
    # Accepted, and quietly dropped rather than stored.
    assert "announcement" not in service.detail(db, sess.user_id, made)

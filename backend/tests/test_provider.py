...
def test_oauth_builds_once_and_reuses(db, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        sf_connect, "connect_oauth", lambda token, user=None, role=None: calls.append(token) or FakeConnection()
    )
    cache = make_cache()
...
def test_oauth_refreshes_expired_token(db, monkeypatch):
    ...
    used = []
    monkeypatch.setattr(
        sf_connect, "connect_oauth", lambda token, user=None, role=None: used.append(token) or FakeConnection()
    )
    cache = make_cache()
    cache.acquire(db, sess)
    assert used == ["at-new"]
...
def test_concurrent_acquire_same_session_leaks_nothing(db_factory, monkeypatch):
    ...
    def fake_connect_oauth(token, user=None, role=None):
        conn = FakeConnection()
        with created_lock:
            created.append(conn)
        barrier.wait()
...
def test_oauth_acquire_marks_its_entry_rebuildable(db, monkeypatch):
    ...
    monkeypatch.setattr(sf_connect, "connect_oauth", lambda token, user=None, role=None: FakeConnection())

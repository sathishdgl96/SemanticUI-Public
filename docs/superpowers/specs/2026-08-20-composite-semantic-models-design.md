...
single view is usable today: the explorer, reports, dashboards, chat,
and Excel. The user's sketch, with the two planners it names made real:
...
```
             User / Power BI / Excel
                  ...
                  backend API
                       |
               SEMANTIC PLANNER          (logical: route fields to views,
                       |                  push filters, pick the join)
             +---------+---------+
             |                   |
       Semantic View A     Semantic View B
             |                   |
             +---------+---------+
                       |
                 QUERY PLANNER           (physical: compile ONE statement)
                       |
                  Snowflake SQL
                       |
                    Results
...
ticket by customer by month" is a legitimate question that today needs a
data engineer to build a third view. The composite model is that third
view expressed as governed configuration instead of new warehouse DDL.
...
**dbt MetricFlow** — metrics belong to semantic models; entities
(keys) are the join graph; the planner builds a dataflow of per-model
...
## 8. Surfacing everywhere a view is usable
...
accepts the composite source; explorer runs it. Integration tests re-run
Phase 0's load-bearing cases.
...
- **No writeback, no new DDL in the warehouse.** A composite is a
  definition — sharing one shares a definition, never numbers.
